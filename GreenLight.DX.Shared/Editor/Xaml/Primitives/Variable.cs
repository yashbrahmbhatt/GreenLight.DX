...
namespace GreenLight.DX.Shared.Editor.Xaml.Primitives
{
    public class Variable
    {
        ...
        public string? Description
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    {
                        VariableElement.Add(new XAttribute(NamespaceNames.X + LocalNames.Description, value));
                    }
                }
            }
        }
        ...
        public string? DefaultValue
        {
            get
            {
                ...
                else
                {
                    ...
                    if (element.Name.LocalName == LocalNames.Literal)
                    {
                        var ValueAttribute = XDocumentHelpers.GetAttribute(element, LocalNames.LiteralValue);
            ...
            set
            {
                ...
                else if (value == "\"\"")
                {
                    if (Variable_Default_ExpressionElement == null || Variable_DefaultElement is XAttribute)
                    {
                        var DefaultElement = new XElement(
                            ...
                            new XElement(
                                NamespaceNames.Empty + LocalNames.Literal,
                                new XAttribute(NamespaceNames.X + LocalNames.VariableType, Type),
                                new XAttribute(LocalNames.LiteralValue, "")
                        ));
                ...
                else if (value.StartsWith("\"") && value.EndsWith("\""))
                {
                    ...
                    if (Variable_Default_ExpressionElement == null)
                    {
                        var DefaultElement = new XElement(
                            ...
                            new XElement(
                                NamespaceNames.Empty + LocalNames.Literal,
                                new XAttribute(NamespaceNames.X + LocalNames.VariableType, Type),
                                new XAttribute(LocalNames.LiteralValue, value.Remove(value.Length - 1).Remove(0, 1))
                        ));
                        VariableElement.Add(DefaultElement);
                    }
                    ...
                    else
                    {
                        ...
                        {
                            case "Literal":
                                var valueElement = XDocumentHelpers.GetAttribute(element, LocalNames.LiteralValue);
                            ...
                            case "CSharpValue":
                                var newElement = new XElement(
                                    NamespaceNames.Empty + LocalNames.Literal,
                                    new XAttribute(NamespaceNames.X + LocalNames.VariableType, Type),
                                    new XAttribute(LocalNames.LiteralValue, value)
                                );
                                element.Remove();
                                ((XElement)Variable_DefaultElement).Add(newElement);
                ...
                else
                {
                    if (Variable_Default_ExpressionElement == null || Variable_Default_ExpressionElement is XAttribute)
                    {
                        var DefaultElement = new XElement(
                            NamespaceNames.Empty + "Variable.Default",
                            new XElement(
                                NamespaceNames.Empty + LocalNames.CSharpValue,
                                new XAttribute(NamespaceNames.X + LocalNames.VariableType, Type),
                                value
                        ));
        ...
        private XAttribute Variable_TypeElement
        {
            get
            {
                return XDocumentHelpers.GetAttribute(VariableElement, LocalNames.VariableType);
            }
        }
        private XAttribute Variable_NameElement
        {
            get
            {
                return XDocumentHelpers.GetAttribute(VariableElement, LocalNames.VariableName);
            }
        }
        ...
        private XAttribute? Variable_DescriptionElement
        {
            get
            {
                return XDocumentHelpers.GetAttribute(VariableElement, LocalNames.Description);
            }
        }
        private XElement ScopeElement
        {
            get
            {
                return XDocumentHelpers.GetClosestParentWithAttribute(VariableElement, LocalNames.DisplayName);
            }
        }
        private XAttribute Scope_NameElement
        {
            get
            {
                return XDocumentHelpers.GetAttribute(ScopeElement, LocalNames.DisplayName);
            }
        }
        ...
        private XAttribute? Variable_Default_Expression_Type
        {
            get
            {
                ...
                return XDocumentHelpers.GetAttribute((XElement)Variable_Default_ExpressionElement, LocalNames.VariableType);
            }
        }
